...
public class TicTacToeServlet extends HttpServlet {
  ...
  public void doGet(HttpServletRequest req, HttpServletResponse resp)
    ...
    if (userService.getCurrentUser() == null) {
      resp.getWriter().println("<p>Please <a href=\"" + userService.createLoginURL(
          getGameUriWithGameParam(req, gameKey)) + "\">sign in</a>.</p>");
      return;
    }
    ...
    // and the user's id.
    String channelId = game.getChannelKey(userId);
    String token = FirebaseAuth.getInstance().createCustomToken(channelId);
    ...
    req.setAttribute("firebase_snippet", game.sFirebaseSnippet);
